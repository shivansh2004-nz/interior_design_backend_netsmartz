...
from utils.cohere_furnitures import get_furniture_list_and_save
...
# from utils.upload_images_to_cloud import upload_assets_and_update_products_csv, upload_cutouts_and_update_products_csv
...
def parse_args():
    p = argparse.ArgumentParser("Interior Design Pipeline (Scrape -> Dataset -> Cutouts -> Upload -> Flux2 -> Cleanup)")
...
def main():
    ...
    # -------------------------
    # Step 1: Furniture list
    # -------------------------
    furnitures = get_furniture_list_and_save(
        room_type=args.room_type,
        style=args.style,
        budget=args.budget,
        out_txt_path=f"{dataset_dir}/Furnitures.txt",
    ...
    if args.skip_cutouts:
        ...
        try:
            ...
            # If cutouts fail, uploading will likely be useless; stop here.
            return
